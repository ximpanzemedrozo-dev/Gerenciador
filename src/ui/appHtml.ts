export const appHtml = `
  ...
  <div id="app-content" class="hidden">
    ...
    <main class="max-w-7xl mx-auto px-4 pt-5 pb-32">
      <section id="view-clients" class="view-section">
        ...
        <div id="clients-bulkbar" class="hidden mb-4 p-3 bg-sky-50 border border-sky-100 rounded-2xl flex justify-between items-center">
           <span class="text-xs font-bold text-sky-700">Selecionados: <span id="clients-bulk-count">0</span></span>
           <div class="flex gap-2">
             ...
             <button onclick="toggleBulkSelectClients(false)" class="text-slate-500 text-[10px] font-bold uppercase">Cancelar</button>
           </div>
        </div>
      ...
      <section id="view-finance" class="view-section hidden">
        ...
        <div id="fin-breakdown" class="space-y-2 bg-white p-6 rounded-2xl border border-slate-200"></div>
      </section>
    </main>

    <nav class="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-950/95 border-t h-20 flex justify-around items-center z-50">
  ...
  <div id="import-modal" class="modal-overlay" onclick="toggleModal('import-modal')">
    <div class="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl p-6" onclick="event.stopPropagation()">
      <h2 class="text-2xl font-black uppercase mb-4">Importação Inteligente</h2>
      <textarea id="import-text" class="w-full h-64 p-4 border rounded-2xl font-mono text-xs mb-4" placeholder="Cole os dados do painel aqui..."></textarea>
      <div id="import-status-area" class="hidden mb-4 p-3 bg-slate-100 rounded-xl">
        <div id="import-status" class="text-[10px] font-bold uppercase">Processando...</div>
        <div class="h-1 bg-slate-200 rounded-full mt-1 overflow-hidden"><div id="import-bar" class="h-full bg-sky-500 w-0"></div></div>
      </div>
      <div class="grid grid-cols-2 gap-3">
        <button onclick="previewImport()" class="bg-slate-800 text-white py-3 rounded-xl font-bold uppercase text-xs">Testar 1º Bloco</button>
        <button onclick="importClientsFromText()" class="bg-sky-500 text-white py-3 rounded-xl font-bold uppercase text-xs">Importar Tudo</button>
      </div>
      <pre id="import-preview" class="mt-4 p-3 bg-slate-50 rounded-xl text-[10px] overflow-auto max-h-32"></pre>
    </div>
  </div>
`;
