...
function refreshTopProfitBar() {
  ...
  if (infoEl) infoEl.textContent = "Filtrando: " + (dashSettings.period === 'current_month' ? 'Mensais do Mês' : 'Dashboard Ativa') + " (" + dashList.length + " itens)";
}
...
window.importClientsFromText = async () => {
  ...
  if (!blocks) return alert("Nenhum cliente válido no texto.");
...
window.handleAuth = async (m) => {
  ...
  try { if (m === "login") await firebaseApi.signInWithEmailAndPassword(auth, e, p); } 
    ...
    if (errEl) { errEl.textContent = "Acesso Negado: " + err.message; errEl.classList.remove("hidden"); }
  }
};
...
window.bulkDeleteSelectedClients = async () => { if(confirm("Apagar " + selectedClientIds.size + " clientes?")) { for(let id of selectedClientIds) await firebaseApi.deleteDoc(firebaseApi.doc(db, "users", currentUserId!, "clients", id)); window.toggleBulkSelectClients(false); } };
