...
// Dashboard: Starplay=2.5, Vision=2.0, Outros=0
function refreshTopProfitBar() {
  ...
  if (infoEl) infoEl.textContent = "Filtrando: " + (dashSettings.period === 'current_month' ? 'Mensais deste Mês' : 'Dashboard Atualizada') + " (" + dashList.length + " clientes)";
}
...
function renderClientsList() {
  ...
  filtered.forEach(c => {
    ...
    div.className = "luxury-card p-5 cursor-pointer border " + (sel ? "ring-2 ring-sky-500 bg-sky-50/20" : "border-slate-200 dark:border-slate-800 shadow-sm transition-all");
...
window.saveClient = async () => {
  ...
  // ATENÇÃO: Caminho corrigido para artifacts/appId/users/userId/clients
...
window.importClientsFromText = async () => {
  ...
  if (!blocks) return alert("Nenhum registro encontrado no texto.");
...
export function installLegacyApp() {
  firebaseApi.onAuthStateChanged(auth, (user) => {
    if (user) {
      ...
      // Sincronizando com o caminho das REGRAS
...
window.handleAuth = async (m) => {
  ...
  try { if (m === "login") await firebaseApi.signInWithEmailAndPassword(auth, e, p); } 
