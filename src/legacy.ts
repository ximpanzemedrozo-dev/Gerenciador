...
let dashSettings = { 
  ...
  apiIAKey: localStorage.getItem("apiIAKey") || ""
};
...
declare global {
  interface Window {
    ...
    importClientsWithAI: () => Promise<void>;
...
// --- NOVA FUNÇÃO: IMPORTAÇÃO COM IA (GEMINI) ---
window.importClientsWithAI = async () => {
  if (!dashSettings.apiIAKey) return alert("Configure sua Chave API na engrenagem primeiro!");
  const text = (document.getElementById("import-text") as HTMLTextAreaElement).value;
  if (!text) return alert("Cole o texto do painel.");

  const btn = document.querySelector('button[onclick*="importClientsWithAI"]') as HTMLButtonElement;
  btn.disabled = true; btn.textContent = "A IA ESTÁ LENDO... 🧠";

  try {
    const prompt = `Extraia os clientes deste texto e retorne APENAS um array JSON puro. Campos: nome, idExt (7 digitos), vencimento (AAAA-MM-DD), plano (numero). Texto: ${text}`;
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${dashSettings.apiIAKey}`, {
      method: "POST",
    ...
    for (let c of parsedData) {
      await firebaseApi.addDoc(firebaseApi.collection(db, "artifacts", appId, "users", currentUserId!, "clients"), {
        nome: c.nome, idExt: String(c.idExt), venc: c.vencimento, plano: Number(c.plano) || 30, painel: "Importado via IA", createdAt: new Date().toISOString()
      });
    }
    alert(`${parsedData.length} clientes limpos e importados com sucesso!`);
    window.toggleModal("import-modal");
  } catch (err) {
    alert("Erro na IA: Verifique sua chave ou o texto.");
    console.error(err);
  } finally {
...
// --- RESTANTE DAS FUNÇÕES MANTIDAS INTEGRALMENTE ---
...
function refreshTopProfitBar() {
  ...
  const dashList = clients.filter(c => {
    ...
    if (dashSettings.period === 'current_month') return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
    if (dashSettings.period === 'custom') {
      const s = dashSettings.startDate ? new Date(dashSettings.startDate + "T00:00:00") : null;
      const e = dashSettings.endDate ? new Date(dashSettings.endDate + "T23:59:59") : null;
      if (s && d < s) return false; if (e && d > e) return false;
    }
    return true;
  });
  const fat = dashList.reduce((acc, c) => acc + (Number(c.plano) || 0), 0);
  const cus = dashList.reduce((acc, c) => {
    const p = c.painel || "";
...
function renderClientsList() {
  ...
  filtered.forEach(c => {
    ...
    div.onclick = () => {
      if (bulkMode) { isSel ? selectedClientIds.delete(c.id) : selectedClientIds.add(c.id); renderClientsList(); }
    };
...
window.saveDashSettings = () => {
  ...
  const key = (document.getElementById('api-ia-key') as HTMLInputElement).value;
  localStorage.setItem("adminPhone", admin); localStorage.setItem("apiIAKey", key);
  dashSettings.adminPhone = admin; dashSettings.apiIAKey = key;
  ...
  window.toggleModal('dash-settings-modal'); refreshTopProfitBar();
};
    ...
    if (user) {
      ...
      firebaseApi.onSnapshot(firebaseApi.collection(db, "artifacts", appId, "users", user.uid, "clients"), snap => {
        ...
        if (grid) {
          ...
          (document.getElementById('api-ia-key') as HTMLInputElement).value = dashSettings.apiIAKey;
        }
      });
...
window.importClientsFromText = async () => { /* Mantida para compatibilidade */ alert("Use o botão de IA para importar o novo formato!"); };
