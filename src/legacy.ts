...
declare global {
  interface Window {
    ...
    refreshFinance: () => void;
    handleDashPeriodChange: (v: string) => void;
...
// ---------- Lógica de Dashboard com Filtro de Data ----------
function refreshTopProfitBar() {
  ...
  // Filtra a lista de clientes para a Dashboard
  const dashList = clients.filter(c => {
    ...
    if (dashSettings.period === 'custom') {
      const start = dashSettings.startDate ? new Date(dashSettings.startDate) : null;
      const end = dashSettings.endDate ? new Date(dashSettings.endDate) : null;
      if (start && d < start) return false;
      if (end && d > end) return false;
      return true;
    }

    return true; // Todos os tempos
  });
  ...
  if (infoEl) {
    let txt = "Filtrando: Base Completa";
    if (dashSettings.period === 'current_month') txt = "Filtrando: Vencimentos deste mês";
    if (dashSettings.period === 'custom') txt = `Filtrando: ${dashSettings.startDate || '?'} até ${dashSettings.endDate || '?'}`;
    infoEl.textContent = `${txt} (${dashList.length} logins encontrados)`;
  }
}

// Funções do Modal da Dashboard
window.handleDashPeriodChange = (v) => {
  const container = document.getElementById("dash-custom-dates");
  if (container) {
    if (v === 'custom') container.classList.remove("hidden");
...
window.saveDashSettings = () => {
  ...
  dashSettings.startDate = (document.getElementById("dash-filter-start") as HTMLInputElement).value;
  dashSettings.endDate = (document.getElementById("dash-filter-end") as HTMLInputElement).value;
